...
def get_workout_plan():
    ...
    try:
        ...
        return jsonify({"error": "Failed to fetch workout plan"}), 500
...
def add_exercise():
    try:
        ...
        min_rep_range = int(data.get("min_rep_range", 0))
        max_rep_range = int(data.get("max_rep_range", 0))
        ...
        with DatabaseHandler() as db:
            query = """
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            """
            params = (routine, exercise, sets, min_rep_range, max_rep_range, rir, weight, rpe)
...
def export_to_workout_log():
    """Export the current workout plan to the workout log."""
    try:
...
@app.route("/update_progression_date", methods=["POST"])
def update_progression_date():
    try:
        ...
        if not all([log_id, new_date]):
            return jsonify({"error": "Missing required fields"}), 400

        query = """
        UPDATE workout_log 
        SET last_progression_date = ? 
        WHERE id = ?
        """
